...
def main():
    """Demonstrate the match explainer functionality."""
    ...
    print("Match Explainer - Demo")
    print("=" * 50)
    
    explanation = explainer.explain_match(poem_analysis, artwork, 0.85, vision_analysis)
    ...
    for connection in explanation['specific_connections']:
        print(f"  • {connection}")
    
    print(f"\nConcrete Matches:")
    concrete = explanation['concrete_matches']
    if concrete['shared_objects']:
        print(f"  • Shared objects: {', '.join(concrete['shared_objects'])}")
    if concrete['setting_alignment']:
        print(f"  • {concrete['setting_alignment']}")
    if concrete['temporal_alignment']:
        print(f"  • {concrete['temporal_alignment']}")
    if concrete['emotional_resonance']:
        print(f"  • {concrete['emotional_resonance']}")
    
    if explanation['potential_tensions']:
        print(f"\nPotential Tensions:")
        for tension in explanation['potential_tensions']:
            print(f"  • {tension}")
